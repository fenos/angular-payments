  ...
  return {
    ...
    link: function(scope, elem, attr) {
      ...
      form.bind('submit', function() {
        ...
        if (scope.stripeDisable === false && angular.isDefined(scope.stripeDisable)) {
          scope.$apply(function() {
              scope.stripeForm({status:200, response: {stripe: false}});
            });

            return
        }
        else {
            expMonthUsed = scope.stripe.expMonth ? true : false;
            expYearUsed = scope.stripe.expYear ? true : false;

            if(!(expMonthUsed && expYearUsed)){
              exp = Common.parseExpiry(scope.stripe.expiry)
              scope.stripe.expMonth = exp.month;
              scope.stripe.expYear = exp.year;
            }
            ...
            if(form.hasClass('ng-valid')) {
              ...
              $window.Stripe.createToken(_getDataToSend(scope.stripe), function() {
                var args = arguments;
                scope.$apply(function() {
                  scope.stripeForm({status:args[0], response: args[1]});
                });
              ...
              scope.$apply(function() {
                scope.stripeForm({status:400, response: {error: 'Invalid form submitted.'}});
              });
              button.prop('disabled', false);
            }

            scope.stripe.expMonth = null;
            scope.stripe.expYear  = null;
        }
